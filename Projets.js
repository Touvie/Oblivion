...
const projectsData = {
    ...
    songproject: {
        title: "Song Project ♪",
        tags: ["Three.js", "WebGL", "MediaPipe", "IA"],
        content: `
            <h3>COMMING ♪</h3>
            <h3>SOON   ♪ ♪ ♪</h3>
            <h3>IN     ♪   ♪  ♪</h3>
...
window.addEventListener('wheel', (e) => {
    // Si la modale est ouverte, on bloque le scroll de la galerie
    if (modal.classList.contains('visible')) return;

    // Si l'utilisateur utilise la molette verticale (deltaY), 
    // on injecte cette valeur dans le défilement horizontal (scrollLeft)
    if (e.deltaY !== 0) {
        e.preventDefault(); // Empêche le comportement par défaut (qui ne ferait rien ici)
        galleryViewport.scrollLeft += e.deltaY;
    }
}, { passive: false }); // "passive: false" est requis pour utiliser e.preventDefault()
...
window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'reset') {
        const galleryViewport = document.querySelector('.gallery-viewport');
        if (galleryViewport) {
            // Remet le défilement horizontal tout à gauche
            galleryViewport.scrollLeft = 0;
            
            // Et on force un événement scroll pour que Three.js (le trou noir) se recale immédiatement
