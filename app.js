...
scene.add(accretionDisk);
        ...
        function setupNavigation() {
            ...
            document.addEventListener('click', (e) => {
                if (e.target.classList.contains('internal-link')) {
                    ...
                    // Écoute les messages des iframes
                window.addEventListener('message', (event) => {
                // Sécurité : ignore si une transition est en cours
                if (isTransitioning || isMovingToSection) return;
                ...
                if (event.data && event.data.type === 'scroll' && event.data.atBoundary) {
                    const now = Date.now();
                    if (now - lastScrollTime < 1500) return; // Même cooldown que ton scroll normal
    ...
    function goToSection(index) {
        // Si on est déjà sur la section et qu'on ne bouge pas, on ne fait rien
        if (index === currentSection && !isTransitioning && !isMovingToSection) return;

        // --- CAS SPÉCIAL : SECTION 1 (À PROPOS) AVEC ANIMATION PERSONALISÉE ---
        if (index === 1) {
            isTransitioning = true;
            ...
            requestAnimationFrame(animateToCenter);
            return; // ON ARRÊTE LÀ POUR LA SECTION 1
        }

        // --- CAS GÉNÉRAL : TOUTES LES AUTRES SECTIONS (0, 2, 3, 4) ---
        isMovingToSection = true;
        ...
        // Reset des scrolls iframes (UX)
        if(index === 2) { // Making Of
            const makeFrame = document.querySelector('.section[data-section="2"] iframe');
            // Lazy loading Making Of si besoin
            if (makeFrame && !makeFrame.getAttribute('src') && makeFrame.dataset.src) {
                makeFrame.src = makeFrame.dataset.src;
            }
        }
        if (index === 3) { // Projets
            const projFrame = document.querySelector('.section[data-section="3"] iframe');
            // Lazy loading Projets si besoin
            if (projFrame && !projFrame.getAttribute('src') && projFrame.dataset.src) {
                projFrame.src = projFrame.dataset.src;
            }
        }

        // Définir la position cible de la caméra
